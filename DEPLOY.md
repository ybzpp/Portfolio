...
Если форма пишет «Не удалось отправить в Telegram» — проверьте токен и chat_id (чат с ботом должен быть начат; для группы бот должен быть добавлен в группу). В разработке можно включить `DEV_SKIP_TELEGRAM=1` в `.env`, тогда форма будет «успешно» отправляться, а заявка выводиться в консоль сервера.

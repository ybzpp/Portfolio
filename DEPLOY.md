...
Если форма пишет «Не удалось отправить в Telegram» — (1) убедитесь, что перезапустили приложение после правки `.env`; (2) проверьте токен и chat_id (для лички сначала напишите боту /start; для группы добавьте бота в группу). В логах сервера будет ответ Telegram (например «Unauthorized» или «chat not found»). В разработке можно включить `DEV_SKIP_TELEGRAM=1` в `.env`, тогда форма будет «успешно» отправляться, а заявка выводиться в консоль сервера.
